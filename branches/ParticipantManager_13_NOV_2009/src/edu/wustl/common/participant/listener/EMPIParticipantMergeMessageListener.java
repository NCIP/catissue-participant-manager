...
public class EMPIParticipantMergeMessageListener
    implements MessageListener
{

    private static Logger logger = Logger.getCommonLogger(EMPIParticipantMergeMessageListener.class);
        ...
        throws DAOException
    {
        ...
        try
        {
            String query = (new StringBuilder()).append("SELECT EMPI_ID FROM CATISSUE_PARTICIPANT WHERE EMPI_ID IN ('").append(pidEmpiId).append("','").append(mrgEmpiId).append("')").toString();
    ...
    {
        String empiId;
        for(empiId = id; empiId.length() > 0 && empiId.charAt(0) == '0'; empiId = empiId.substring(1)) { }
        return empiId;
    }

    private void storeMergeMessage(JDBCDAO jdbcdao, String hl7Message, String messageType, String pidEmpiId, String mrgEmpiId)
        throws DAOException
    {
        long idenifier = 0L;
        String insQuery = "";
